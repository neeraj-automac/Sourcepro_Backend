...
def Next_lesson(request):
    ...
    if user_lesson_queryset is not None:
        # print(user_lesson_queryset.lesson_id)
        # print(user_lesson_queryset.course_id.pk)
        # print('queryset',user_lesson_queryset)

    # if not user_lesson_queryset.exists():
    #     next_lesson_id=next_lesson_id+1




    # info1 = Lessons.objects.filter(course_in_lessons = course_id)
    # print(info1)
    # for lesson in info1:
    #     print('lesson',lesson)
    #     # Access materials associated with the current lesson using the reverse relation defined in the Material model
    #     materials_for_lesson = lesson.materials.all()
    #     print('materials_for_lesson',materials_for_lesson)
    #     # Now you can do whatever you want with the materials_for_lesson
    #     # For example, you can print the names of the materials associated with the current lesson:
    #     for material in materials_for_lesson:
    #
    #         print('material_and_url',material,material.material_url)
    # # print(a)

        return JsonResponse({"course_id":user_lesson_queryset.course_id.pk,"next_lesson_id" :user_lesson_queryset.lesson_id})
    # else:
    #     print('Lessons.objects.all()',Lessons.objects.all())
    #     for i in range(len(Lessons.objects.all())):
    #     # while True:
    #         user_lesson_queryset = Lessons.objects.filter(course_in_lessons=course_id, lesson_id=lesson_id).order_by('lesson_id').first()
    #         print('user_lesson_queryset****',user_lesson_queryset)
    else:
...
def usr_course_page(request):
    ...
    if request.method=='GET':
        ...
        if current_user_courses.exists():
            ...
            for i in range(0,len(serializer_data)):
                ...
                # print('ressssssssssss',course_data_result)
            # for i in range(0,len(faq_queryset)):
            #     print(i,faq_queryset)
            #     course_data_result.update({"question"+str(i+1): faq_queryset[i].question,"answer"+str(i+1): faq_queryset[i].answer})#,i+1: faq_queryset[i].answer
            ...
            # current_user_lesson=User_Lessons.objects.filter(user_id=user_id,course=course_id)
            # print('current_user_lesson', current_user_lesson)
            # # print('current_user_lesson lesson name', current_user_lesson[0].course_lesson_id.lesson_name)
            #
            # lesson_details_serializer=User_Lessons_info_serializer(current_user_lesson,many=True)
            # lesson_details_serializer_data=lesson_details_serializer.data
            # print('lesson_details_serializer_data',lesson_details_serializer_data)
            #
            #
            # for i in range(0,len(current_user_lesson)):
            #     print('current_user_lesson lesson name', current_user_lesson[i].course_lesson_id.lesson_name)
            ...
            #     lesson_details_serializer_data[i].update({
            #         'lesson_name':current_user_lesson[i].course_lesson_id.lesson_name,
            ...
            # print(current_user_lesson_material)
            # current_user_lesson_material_serializer=User_Lessons_material_serializer(current_user_lesson_material,many=True)
            ...
            # print(current_user_lesson_material)
            # current_user_lesson_clipboard_serializer = User_Lessons_clipboard_serializer(current_user_lesson_clipboard, many=True)
            # current_user_lesson_clipboard_serializer_data = current_user_lesson_clipboard_serializer.data
            # print('current_user_lesson_material_serializer', current_user_lesson_clipboard_serializer_data)

            # for i in range(0, len(current_user_lesson_material)):
            #     print('current_user_lesson lesson materials', current_user_lesson_material[i].lesson.material_url)
            #     print('current_user_lesson lesson clipboards', current_user_lesson_material[i].lesson.clipboard_url)
    ...
    #     d_lesson_watch_time=datetime.strptime(lesson_watch_time, '%H:%M:%S').time()
    ...
    #
    #     # l=request.data
    #     old_watch_time=User_Lessons.objects.get(course_lesson_id=lesson_id)
    #     print('old_watch_time',old_watch_time)
    #     old_watch_time_serializer = User_Lessons_serializer_update_watch_time(old_watch_time,data=converted_lesson_watch_time)
    #
    #
    #
...
def course_page_lesson_id(user_id,course_id,lesson_id):
    ...
    lesson_material_list.append({'material_name': current_user_lesson.lesson_id.lesson_name})
    ...
    lesson_clipboard_list.append({'clipboard_name': current_user_lesson.lesson_id.lesson_name})
...
def quiz_attempt(request):
    ...
    if serializer.is_valid():
        ...
        course_status_queryset=User_Lessons.objects.filter(user_id=2,course_id=request.data.get('course_id'))

        course_status_update=usr_course.objects.get(user_id=2, course_id=request.data.get('course_id'))
...
def user_details(request):
    ...
    if request.method == 'GET':
        ...
        try:
            rec = User_details.objects.get(user_id = 2)#ds["user_id"]
            ...
            de = User_details.objects.filter(user_id=2)#ds["user_id"]
...
#     if request.user.is_authenticated:
    ...
    if serializer.is_valid():
        ...
        # user = request.user
        user = User.objects.get(username="neerajpynam3@gmail.com")
        # print(user)
        ...
        if new_password == current_password:
            return JsonResponse({"status": "New password cannot be the same as the old password"})
